...
def to_tuple(lst):
    return tuple(to_tuple(i) if isinstance(i, (list, set, tuple,np.ndarray)) else i for i in lst)


def find_intersection(segments):
    segments=segments if isinstance(segments[0], tuple) else to_tuple(segments)

    isector = SweepIntersector()
    isec_dic = isector.findIntersections(segments)
    return  {k:v[1:][:-1] for k,v in isec_dic.items()}
def polygon_segments(polygon):

    n = len(polygon)
    for i in range(n):
        start = polygon[i]
        end = polygon[(i + 1) % n]  # wrap around to the first vertex
        yield start, end
