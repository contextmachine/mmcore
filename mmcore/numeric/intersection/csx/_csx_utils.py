...
from mmcore.geom._nurbs_eval import evaluate_nurbs_surface, NURBSSurfaceTuple, NURBSCurveTuple

from mmcore.numeric import gram_schmidt
...
def refine_intersection_point(
    x: np.ndarray, curve: NURBSCurveTuple, surf2: NURBSSurfaceTuple, spt: float = 1e-3, eps_n=None, angle_tol=0.052, max_iter: int = 10
    ...
    while iteration < max_iter:
        ...
        p_eval = evaluate_nurbs_curve(curve, s, d_order=2)
        q_eval = evaluate_nurbs_surface(surf2, u, v, d_order=1)
        
        S0 = np.array(p_eval["C"])
        S1 = np.array(q_eval["S"])
        error = np.linalg.norm(S0 - S1)
        n1=np.asarray(gram_schmidt(p_eval["C1"],p_eval["C2"]))
        ...
        # Check convergence.
        if (error < spt) and within_normal_gap(n1, n2, p_eval["C"], q_eval["S"], angle_tol, eps_n):
