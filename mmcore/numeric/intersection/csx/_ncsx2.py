...
class CSInt:
    ...
    def compare_with_tol(self, tuv:NDArray[float], curve_tuple: NURBSCurveTuple = None, surface_tuple: NURBSSurfaceTuple = None):
        ...
        if not within_tol[2]:  # v parameter failed standard tolerance
            ...
            v_closed_boundary = (abs(v_self - v0) <= dv and abs(v_other - v1) <= dv) or \
                               (abs(v_self - v1) <= dv and abs(v_other - v0) <= dv)
    ...
    def _check_geometric_continuity(self, other_tuv: NDArray[float], curve_tuple: NURBSCurveTuple, surface_tuple: NURBSSurfaceTuple, failed_params: list, eps: float = 1e-3) -> bool:
        """
        ...
        """
        try:
            # Evaluate curve and surface at other point with derivatives
            ...
            # For boundary cases on closed surfaces, focus on position continuity
            # Compare positions (most important for closed surfaces)
            ...
            if curve_pos_diff > eps or surf_pos_diff > eps:
                return False

            # For boundary cases, be more lenient with derivative checks
            # Only check derivatives if the failing parameters might affect curve tangency
            if 't' in failed_params:
                # If curve parameter failed and is at boundary, allow more tolerance for curve tangent
                deriv_eps = eps * 100  # Much more lenient for boundary curve tangent
                curve_d1_diff = np.linalg.norm(self.curve_eval["C1"] - other_curve_eval["C1"])
                if curve_d1_diff > deriv_eps:
                    return False
            
            # For surface parameters u,v at boundaries, focus only on position continuity
            # since closed surfaces can have legitimately different derivatives at boundaries
            
            return True

        except Exception as err:
            logger.error(f"Error during geometric continuity check: {err}")
                ...
                for index ,  existing_inter in enumerate(inters):
                    ...
                    if existing_inter.compare_with_tol(tuv, original_curve, original_surface):
                        ...
                        if existing_inter.error > error:
                            ...
                            logger.debug(
                                "Pass (the error of the new tuv exceeds the error of the existing ones, {}<={}) : {}".format(
                                    existing_inter.error, error, tuv
                                )
                            )
                            logger.debug("Inters: {}".format([i.tuv for i in inters]))
