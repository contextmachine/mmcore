...
def trace_overlap(
    ...
    crv: NURBSSurfaceTuple,
    surf: NURBSSurfaceTuple,
    spt=1e-3,
    angle_tol=0.052,
):
    crv_n = _tuple_to_nurbs(crv)
    srf_n = _tuple_to_nurbs(surf)
    t0, t1 = _curve_interval(crv)
    (u0, u1), (v0, v1) = _surface_interval(surf)

    def evaluate_crv_data(t):
        crv_eval = evaluate_nurbs_curve(crv, t, d_order=3)
        kws = {**crv_eval}
        del kws["C"]
        crv_eval["dt"] = compute_parametric_tolerance_curve(**kws, spt=spt, angle_tol=angle_tol)
    ...
    def evaluate_surf_data(u, v):
        ...
        kws = {**surf_eval}
        del kws["S"]
        du, dv = compute_parametric_tolerance_surface(**kws, spt=spt, angle_tol=angle_tol)
        surf_eval["du"] = du
        surf_eval["dv"] = dv
