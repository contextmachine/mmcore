...
def to_scaled(ctrl: np.ndarray) -> np.ndarray:
    ...
    """
    n = len(ctrl) - 1
    scale = np.fromiter((math.comb(n, i) for i in range(n + 1)),
...
def compose_patch_curve_sb(patch_ctrl: np.ndarray,
...
def compose_patch_curve_rational(patch_ctrl: np.ndarray,
