...
cdef class NURBSCurve(ParametricCurve):
    ...
    def  flip(self):
        return NURBSCurve(np.flip(self._control_points,axis=0), self.degree,np.max(self.knots)-self.knots+np.min(self.knots))
...
@cython.wraparound(False)
@cython.cdivision(True)
def can_join_curves(curve1: NURBSCurve, curve2: NURBSCurve, tol: float = 1e-7) -> bool:
    """
    Check if two NURBS curves can be joined.

    Parameters:
    -----------
    curve1: NURBSCurve
        First curve to check
    curve2: NURBSCurve
        Second curve to check
    tol: float
        Tolerance for geometric comparison

    Returns:
    --------
    bool: True if curves can be joined, False otherwise
    """
    # Degrees can be different - we'll handle that during joining

    # Get all endpoints
    start_point1 = curve1.evaluate(curve1._interval[0])
    end_point1 = curve1.evaluate(curve1._interval[1])
    start_point2 = curve2.evaluate(curve2._interval[0])
    end_point2 = curve2.evaluate(curve2._interval[1])

    # Calculate distances between possible connecting points
    distance_end1_start2 = sqrt(
        pow(end_point1[0] - start_point2[0], 2) +
        pow(end_point1[1] - start_point2[1], 2) +
        pow(end_point1[2] - start_point2[2], 2)
    )

    distance_end1_end2 = sqrt(
        pow(end_point1[0] - end_point2[0], 2) +
        pow(end_point1[1] - end_point2[1], 2) +
        pow(end_point1[2] - end_point2[2], 2)
    )

    # Take minimum distance
    distance = min(distance_end1_start2, distance_end1_end2)
...
def reverse_curve(curve: NURBSCurve) -> NURBSCurve:
    """
    Create a new curve with reversed direction.
    ...
    Returns:
    --------
    NURBSCurve: New reversed curve
    """
    # Reverse control points
    cdef double[:,:] new_cpts = np.flip(curve._control_points, axis=0).copy()
    ...
    return NURBSCurve.create(new_cpts, curve._degree, new_knots, curve._periodic)

@cython.boundscheck(False)
@cython.wraparound(False)
@cython.cdivision(True)
def join_two_curves(curve1: NURBSCurve, curve2: NURBSCurve) -> NURBSCurve:
    """
    Join two compatible NURBS curves, handling different degrees through degree elevation.
    ...
    curve1: NURBSCurve
        First curve to join
    curve2: NURBSCurve
        Second curve to join

    Returns:
    --------
    NURBSCurve: New joined curve
    """
    # Check orientation and reverse second curve if needed
    start_point1 = curve1.evaluate(curve1._interval[0])
    end_point1 = curve1.evaluate(curve1._interval[1])
    start_point2 = curve2.evaluate(curve2._interval[0])
    end_point2 = curve2.evaluate(curve2._interval[1])
    ...
    # Reverse curve2 if its end point is closer to curve1's end point
    curve2_modified = curve2.flip() if distance_end1_end2 < distance_end1_start2 else curve2

    # Handle different degrees by elevating the lower degree curve
    if curve1._degree != curve2_modified._degree:
        if curve1._degree < curve2._degree:
            # Elevate first curve to match second
            t = curve2._degree - curve1._degree
            curve1 = degree_elevate_curve(curve1, t)
        else:
            # Elevate second curve to match first
            t = curve1._degree - curve2._degree
            curve2 = degree_elevate_curve(curve2, t)

    degree = curve1._degree  # Now both curves have same degree

    # Get control points and weights
    cpts1 = curve1._control_points
    cpts2 = curve2._control_points

    # Create new knot vector by joining and scaling
    knots1 = curve1._knots
    knots2 = curve2._knots

    # Scale second knot vector to start where first one ends
    scale2 = knots1[-1] - knots2[0]
    new_knots2 = np.array([k + scale2 for k in knots2[degree+1:]], dtype=np.float64)
    ...
    # Create new curve
    return NURBSCurve.create(new_cpts, degree, new_knots, False)

@cython.boundscheck(False)
@cython.wraparound(False)
@cython.cdivision(True)
def find_connected_curves(curves: list, tol: float = 1e-7) -> list:
    """
    Group curves that can be joined together.

    Parameters:
    -----------
    curves: list
        List of NURBSCurve objects
    tol: float
        Tolerance for geometric comparison

    Returns:
    --------
    list: List of lists, where each inner list contains connected curves
    """
    if not curves:
        return []

    # Create graph of connected curves
    n = len(curves)
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if can_join_curves(curves[i], curves[j], tol):
                connections[i].add(j)
                connections[j].add(i)
    ...
    for i in range(n):
        if i not in visited:
            ...
            components.append([curves[idx] for idx in current_component])

    return components

@cython.boundscheck(False)
@cython.cdivision(True)
def join_curves(curves: list, tol: float = 1e-7) -> list:
    """
    Join multiple NURBS curves where possible.

    Parameters:
    -----------
    curves: list
        List of NURBSCurve objects to join
    tol: float
        Tolerance for geometric comparison

    Returns:
    --------
    list: List of joined curves. If all curves could be joined,
          returns a list with a single curve. Otherwise returns
          list of curves that could be joined within their groups.
    """
    if not curves:
        return []
    if len(curves) == 1:
    ...
    # Find groups of connected curves
    connected_groups = find_connected_curves(curves, tol)
    ...
    for group in connected_groups:
        ...
        # Sort curves in joining order
        sorted_group = [group[0]]
        remaining = set(group[1:])

        while remaining:
            current = sorted_group[-1]
            next_curve = None

            # Find next curve to join
            for curve in remaining:
                if can_join_curves(current, curve, tol):
                    next_curve = curve
                    break

            if next_curve is None:
                # No more curves can be joined to current sequence
                break

            sorted_group.append(next_curve)
            remaining.remove(next_curve)

        # Join the sorted group
        joined = sorted_group[0]
        for i in range(1, len(sorted_group)):
            joined = join_two_curves(joined, sorted_group[i])
