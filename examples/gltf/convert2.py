...
from mmcore.geom.mesh import build_mesh_with_buffer, union_mesh, union_mesh2, union_mesh_old, vertexMaterial
...
def get_parts(conts):
    for c in conts:
        yield from requests.get(f"https://viewer.contextmachine.online/cxm/api/v2/{c}/zone-scopes").json()


def get_stats(conts):
    for c in conts:
        yield from requests.get(f"https://viewer.contextmachine.online/cxm/api/v2/{c}/stats").json()
