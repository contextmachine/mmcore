...
2. **L52 slice 10** — A2 leads: (a) csx `>12` continuum-chain constant — build the short-exact-span (≲12·ptol_t) fixture and justify/replace the constant; (b) 65-grid even-crossing aliasing — assess/document.
3. **L52 slice 12 remainder** — §10 efficiency items (ccx phase-2 AABB-before-hull ordering, strict-polish seeds, `_choose_phi_equations` tensor rebuilds, O(B²) identity-mapping attempts, loop-invariant `_strict_ssx_root_tol`, overlap-postprocess after denial); flag zoo (`hard_exhausted`/`output_counts` have zero readers).
4. **L50** — investigated-open: fix ONLY with a reaching two-tangency fixture (analysis + fix direction in the ledger).
