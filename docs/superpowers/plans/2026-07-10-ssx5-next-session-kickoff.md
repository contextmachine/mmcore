...
**Next session:**
1. **L52 remainder**: slice 7 (kit-helper duplicates: centering/context ×2, strict gates ×2, restrictors ×~6, collapsed-geometry predicate ×3, + the ccx depth-margin vs ssx K=128 policy documentation); slice 8 (`tol=`/`atol=` kwarg hygiene + reject unknown kwargs; Optional-budget threading residue — the 41 `is not None` guards); slice 9 (**the substantive one**: c1 tier §11.6 de-budget — `min(20_000, remaining)` at the c1 call site saturates on case 14; the L49 permanent `work_budget` misbilling on positive-dim Σ (5,693 of 1M cells); typed case-13 complement; then declare+enforce "no gate case reports `work_budget`"); slice 10 (A2 leads: csx `>12` chain constant needs a short-span fixture at `_bez_csx4.py` ~1643; 65-grid aliasing); slice 12 remainder (efficiency items §10; flag zoo `hard_exhausted`/`output_counts`).
...
.venv/bin/python -m pytest tests/test_bez_csx4.py tests/test_bez_ccx4.py tests/test_bez_ccx3_cases.py tests/test_bezier_common.py tests/test_bezier_curves_overlap.py tests/test_nurbs_param_tol_regression.py tests/test_ccx4_exactness_contract.py tests/test_csx4_exactness_contract.py tests/test_work_budget.py -q   # exactness contracts + budget contracts joined the batch 2026-07-12 (L56: they went stale-red silently while outside the gates)
```
