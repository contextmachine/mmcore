...
>
> **REVIEW FOLLOW-UPS (this commit; review of `2d030bb`+`7ed47c0`):** (a) **Budget-starvation blind band FOUND and FIXED**: `_emit_offcurve_tangent_roots`' per-pop `max_cells` charging let the on-curve flood's hull-excluded siblings starve the budget (measured 1582 excluded vs 0 touch-boxes charged) — off-curve touches at 5–15·atol from the tangent line were silently lost (family `z=(2t−1)²((s−0.7)²+(t−(0.5−δ))²)`; found at δ=20·atol, lost at 15/12.5/10/5·atol). Fix: `solve_zero_dim` skip-aware budget — with `skip_newton` given, `max_cells` charges ONLY actual Newton attempts; traversal bounded by `boxes ≤ max_cells + 16·cells` (a pure no-frontier flood stops after exactly `max_cells` boxes = the historical traversal) plus hard `max_boxes` (default 16·max_cells); `exhausted=True` on any cap. No-skip callers charge per-pop as before (bit-identical). Measured post-fix: all δ ∈ {5,10,12.5,15,20}·atol found at default budget; δ=10·atol repro 3.75s; legacy tangential 319ms vs 383ms pre-fix (the flood got cheaper: Δ-enumeration Newton = dedicated float GN on a degree-elevated stacked T net per plan Risk-7, was 77% of runtime through the generic interval-capable evaluator; G/T nets ride as `VectorBoxNet` bundles, 3 de Casteljau splits per box instead of 8; F_sq first in exclusion order). (b) The earlier "budget exhaustion starves only the curve flood" claim was FALSE pre-fix; docstrings now state the actual guarantee. (c) The (2) "load-bearing filter" claim was overstated: the 3×Ψ hull-exclusion nets in `phi_loop_seeds` kill the sub-tolerance valley UPSTREAM (it never seeds); the full-Ψ refinement is the second line of defense (degenerate-line samples, near-loop seed error) — docstring reworded. (d) **KNOWN LIMIT recorded in the arm's comment + `_emit_offcurve_tangent_roots` docstring**: the off-curve enumeration recovers only Δ-roots; a coexisting TRANSVERSAL loop (not on Δ, no boundary crossings) in the same crossing-bearing cell is still lost (the arm `continue`s without subdividing; Φ∩L seeding runs only on the crossing-less arm). (e) `_march_phi_closed` now re-checks connectivity after the Ψ-validity keep-filter (kept samples must form ONE cyclic run, else fail → ranked-equation retry, matching `_choose_phi_equations`' documented contract); the vacuous 8·ptol closure re-checks in both closed marchers replaced by the exactness invariant; final SSXPoint dedup (unify_tol box AND 2·atol xyz) added at the result filter.
