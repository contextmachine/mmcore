...
def test_ssx1():
    """
    This test use a simple check against pre-recorded 'correct points'
    and if any of the ssx components are modified, the tests may well give false negatives.
    ...
    _compare_with_expected_result(result, "ssx-test1.json")
...
def test_ssx2():
    ...
    _compare_with_expected_result(result, "ssx-test2.json")



def _compare_with_expected_result(ssx_result,file_name:str):

    with (testdir /file_name).open('r') as f:
        expected_result = json.load(f)

    for i, (spatial, uv1, uv2) in enumerate(ssx_result):
        print(f'\t{i + 1}. {spatial}, {uv1}, {uv2}')
        assert np.allclose(spatial.control_points, expected_result[i][0])
        assert spatial.degree == expected_result[i][1]