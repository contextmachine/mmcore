...
(kept test-local by decision; promote to sbern only when a second consumer
appears — docs/RESTRUCTURE.md §7.1, NEW-Q3).

What this file uniquely pins (not covered by test_nurbs_curve_compose.py):
scalar Bernstein composition against closed form (t²∘t² = t⁴), power-basis
root extraction on an interval, composition breakpoints from knot crossings,
B-spline → Bezier span extraction, and the nurbs_bezier_to_bern /
bern_to_nurbs_bezier round trip through a rational composition.
"""
...
def find_polynomial_roots_in_interval(coeffs, lo, hi):
    """Real roots of a power-basis polynomial (ascending coeffs) inside [lo, hi]."""
    return sorted(r.real for r in np.roots(coeffs[::-1])
                  if abs(r.imag) < 1e-12 and lo <= r.real <= hi)


def compose_bernstein_polynomials(outer, inner):
    """Exact scalar Bernstein composition via homogeneous compose_curve_curve."""
    h = compose_curve_curve(np.column_stack([outer, np.ones(len(outer))]),
                            np.column_stack([inner, np.ones(len(inner))]))
    return h[:, 0] / h[:, 1]
...
class BezierSegment:
    def __init__(self, seg):
        self.seg = seg
        self.degree = seg.order - 1
        self.control_points = seg.control_points
        self.weights = seg.weights
        self.start_param, self.end_param = _segment_interval(seg)


def extract_bezier_from_bspline(curve, t0, t1):
    for seg in decompose_curve(curve):
        s0, s1 = _segment_interval(seg)
        if abs(s0 - t0) < 1e-12 and abs(s1 - t1) < 1e-12:
            return BezierSegment(seg)
    raise ValueError(f"no Bezier span [{t0},{t1}]")


def find_composition_breakpoints(f_curve, c_knots):
    """Breakpoints of C(f(t)): interior crossings plus the domain endpoints.

    sbern's _collect_split_parameters is deliberately strictly interior
    (its caller must not be handed endpoint splits); the endpoints are
    recovered here from the segment interval.
    """
    d = f_curve.order - 1
    interior = np.unique(c_knots[d + 1:-(d + 1)]) if len(c_knots) > 2 * (d + 1) else np.unique(c_knots[1:-1])
    t0, t1 = _segment_interval(f_curve)
    return [t0] + list(_collect_split_parameters(f_curve, interior, np.array([]))) + [t1]
...
class TestNURBSComposition:
    ...
    def test_quadratic_reparameterization(self):
        ...
        # Quadratic f(t) = t^2: Bezier control points [0, 0, 1]
        f_knots = np.array([0.0, 0.0, 0.0, 1.0, 1.0, 1.0])
        f_control = np.array([[0.0], [0.0], [1.0]])
