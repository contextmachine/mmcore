...
class TestPolyCurve(TestCase):
    def setUp(self) -> None:
        ...
        self.eq1 = np.array(
                [[-22.04779136, -0.83248855, 0.], [-9.38051085, -28.66071821, 0.], [22.84625293, -27.4081778, 0.],
    ...
    def test_props(self):
        ...
        self.assertTrue(np.allclose(self.poly.units, np.array(
                [[0.41429309, -0.91014352, 0.], [0.99924555, 0.03883714, 0.], [-0.24856184, 0.96861603, 0.],
                 [-0.99597107, -0.0896751, 0.]]
                )
                                    )
                        )
    ...
    def test_mbr_normal(self):
        self.assertTrue(np.allclose(self.poly.mbr.normal - np.array([0., 0., 1.]), 0.))
